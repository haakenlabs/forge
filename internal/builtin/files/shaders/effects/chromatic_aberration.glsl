...
vec4 pass_0()
{
    ...
    vec2 max_distort = vec2(max_distort_px) / u_resolution;
    ...
    for ( int i=0; i<num_iter;++i )
    {
        ...
        sumw += w;
        sumcol += w * srgb2lin(texture(u_source, distort(uv, t, min_distort, max_distort )).rgb);
    }
    sumcol.rgb /= sumw;
